...
export class DocumentSequenceService {
  ...
  async getNextNumber(companyId: string, module: string, docType: string, prefix: string): Promise<string> {
  ...
  async resetSequence(companyId: string, module: string, docType: string): Promise<void> {
  ...
  async updateSequence(companyId: string, module: string, docType: string, data: { prefix?: string; padding?: number }): Promise<void> {
    await this.prisma.documentSequence.updateMany({
      where: { companyId, module, docType },
      data,
    });
